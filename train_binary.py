...
df = pd.read_csv("orientation/orientation-gb-train.tsv", sep="\t")
print(f"Total rows: {len(df)}")
print(f"Label distribution: {df['label'].value_counts().to_dict()}")

df = df[df["label"].isin([0, 1])].copy()
df["text"] = df["text_en"].fillna(df["text"]).astype(str)
...
for epoch in range(EPOCHS):
    ...
    if accuracy > best_accuracy:
        ...
        model.save_pretrained("uk_left_right_binary")
        tokenizer.save_pretrained("uk_left_right_binary")
        print(f"  -> Best model saved!")

print(f"\nBest validation accuracy: {best_accuracy:.4f}")
print("Model saved to uk_left_right_binary/")
